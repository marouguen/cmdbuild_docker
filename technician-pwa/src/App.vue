...
const installPrompt = (event: Event) => {
  ...
  install.value = event as typeof install.value;
};
...
<template>
  <header class="app-header">
    <div class="brand">
      <span class="brand-mark" aria-hidden="true">{{ appShortName.charAt(0).toUpperCase() }}</span>
      <span>{{ appShortName.toUpperCase() }}<small>CMMS</small></span>
    </div>
    <div class="header-actions">
      <div class="lang-switch" role="group" aria-label="Language / Langue">
        <button type="button" class="header-button" :class="{selected: locale === 'en'}" :aria-pressed="locale === 'en'" @click="setLocale('en')">EN</button>
        <button type="button" class="header-button" :class="{selected: locale === 'fr'}" :aria-pressed="locale === 'fr'" @click="setLocale('fr')">FR</button>
      </div>
      <button v-if="install" class="header-button" @click="install.prompt(); install = null">{{ $t('app.install') }}</button>
      <button v-if="session" class="header-button" :disabled="busy" @click="logout">{{ $t('app.signOut') }}</button>
    </div>
  </header>
  ...
  <main :class="{login: !session}">
    <template v-if="!session">
      <div class="welcome">
        ...
        <p>{{ $t('login.sub1') }}<br>{{ $t('login.sub2') }}</p>
      </div>
      <form class="panel login-form" @submit.prevent="login">
        ...
        <fieldset :disabled="busy || !online">
          ...
          <label>{{ $t('login.username') }}<input v-model="username" autocomplete="username" autocapitalize="none" spellcheck="false" required></label>
          <label>{{ $t('login.password') }}<input v-model="password" type="password" autocomplete="current-password" required></label>
          <button class="primary wide">{{ busy ? $t('login.signingIn') : $t('login.signIn') }}</button>
        </fieldset>
        <p v-if="error" class="error" role="alert">{{ error }}</p>
        <p v-if="notice" class="hint" role="status">{{ translateError(notice) }}</p>
      </form>
      <p class="login-foot">{{ $t('app.connectedTo') }}<template v-if="deploymentName"> · {{ deploymentName }}</template></p>
    </template>
    <AppShell v-else :session="session" />
  </main>
  <footer v-if="session" class="app-footer">{{ $t('app.systemOfRecord') }}</footer>
</template>
